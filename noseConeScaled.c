 ...
 * noseConeScaled.c
 *
 * Using glScalef() as a design tool.
 * 
 * Exactly the same nose cone is made as in noseCone.c: the first piece is 
 * still a circular cylinder, while the second piece is now a hemisphere that
 * is elongated by scaling to become exactly half of a circular ellipsoid.
 */
...
// a hemisphere with a circular cross-section parallel to the xy-plane.  
...
void init (void) 
{
  ...
  // Fill the second vertex array with hemisphere points.
  k = 0;
  for (j = 0; j <= N; j++)
    for (i = 0; i <= M; i++){
      vdata2[k++] = FN2X(i,j); 
...
void display(void)
{
   ...
   // Translate the elongated hemisphere to attach to the cylinder.
   glTranslatef(0.0, 0.0, 3.0);
   
   glPushMatrix();

   // Elongate the hemisphere to turn it into half of an ellipsoid.
